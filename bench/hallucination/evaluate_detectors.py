...
def main():
    ...
    ap.add_argument("--detector", action="append", required=True, help="llm:<model> or transformer:<path>[:<taxonomy_head>]")
    ap.add_argument("--dataset", default="halueval")
    ap.add_argument("--max-samples", type=int, default=None)
    ap.add_argument("--base-url", default=None, help="OpenAI-compatible endpoint for llm detectors (e.g. vLLM)")
    args = ap.parse_args()
    ...
    if len(all_metrics) > 1:
        ...
        for spec, m in all_metrics.items():
            ...
            print(
                ...
                f"  char F1={c['f1']:.3f}" if c else f"{spec}\n  example F1={e['f1']:.3f}",
            )
            if lat:
