...
def answer_token_probs(tok, model, device, context, question, answer):
    ...
    enc = tok(prefix + answer, return_tensors="pt", truncation=True,
              max_length=8192, return_offsets_mapping=True)
    offsets = enc.pop("offset_mapping")[0].tolist()
    enc = {k: v.to(device) for k, v in enc.items()}
    with torch.no_grad():
        probs = torch.softmax(model(**enc).logits[0], dim=-1)[:, 1].tolist()
    return [p for p, (s, e) in zip(probs, offsets) if e > s and s >= len(prefix)]
...
def metrics(rows, threshold, min_span):
    ...
    return {
        "threshold": threshold, "min_span": min_span,
...
def main():
    ...
    d_model = AutoModelForTokenClassification.from_pretrained(DETECTOR).to(device).eval()
    if not args.no_sentinel:
        s_tok = AutoTokenizer.from_pretrained(SENTINEL)
        s_model = AutoModelForSequenceClassification.from_pretrained(SENTINEL).to(device).eval()
    ...
    for i, item in enumerate(ds):
        ...
        needs_check = True
        if not args.no_sentinel:
            enc = s_tok(item["question"], return_tensors="pt", truncation=True,
        ...
        probs = answer_token_probs(d_tok, d_model, device,
                                   item["knowledge"], item["question"], item["answer"])
